...
\end{parts}
\clearpage
\sectionquestion{Precision and Recall}
\begin{parts}

\part[1] \sone Maru is considering using an ML model to classify regions of the human genome based on whether or not they are likely to contribute to a specific type of cancer. After collecting the predictions from a held-out test set, he asks his colleagues to run experiments in the lab to verify the regions predicted to be positive, since verifying negative predictions is not useful to his work. If only predicted positive regions receive lab evaluations to determine their true labels, which of the following metrics can Maru compute?
...
\part Keria is training a model to classify jeans as new $(y=0)$ or old $(y=1)$.
He plans to send the old jeans to get repaired, but he is only able to send in $k$ jeans out of a batch of $100$ jeans for repairs due to budget constraints, even though Keria knows that exactly $20$ pairs in each batch are old.
Keria thus trains a probabilistic classifier (which outputs a prediction $\widehat{y} \in [0, 1]$ for each item in the batch) and selects the jeans with the top $k$ predicted values $\widehat{y}$ to send in from each batch.

\begin{subparts}

\subpart[1] \sall Suppose $k=10$. Which of the following metrics are good measures of performance for Keria's model for the described application?

{\checkboxchar{$\Box$} \checkedchar{$\blacksquare$}
\begin{checkboxes}
% YOUR ANSWER
% Change \choice to \CorrectChoice for the appropriate selection/selections
\choice Precision (since Keria needs to make sure that the jeans that are sent in are actually old)
\choice Recall (since Keria needs to send in as many of the old jeans as possible)
\choice Accuracy (since Keria needs the model to be correct on both jeans sent in and jeans not sent in)
\choice None of the above
\end{checkboxes}
}

\subpart On a given batch of jeans, Keria evaluates his model's precision and recall when selecting $k$ jeans to repair for $k \in \{10, 20, \dots, 100\}$.
Mark each of the following plots as possible or impossible plots of precision and recall for Keria's model and, if marking impossible, explain why.
...
\begin{your_solution}[title=Explanation, height=3cm, width=0.8\textwidth]
\end{your_solution}
